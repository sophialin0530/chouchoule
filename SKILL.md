...
2. **自动生成「10 个普通兑换码 + 1 个万分码」，交付时必须连同链接/压缩包一起给用户。** 普通码默认 500 积分；万分码一次到账 10000。**码名默认为英文+数字组合**：脚本从网站名提取英文字母作前缀（如「布布Garden」→`GARDEN01`~`GARDEN10`+`GARDENWANFEN`）；**纯中文站名无英文部分时回退通用前缀 `GACHA`**（如「紫雾花园」→`GACHA01`~`GACHA10`+`GACHAWANFEN`，构建时会打印提示）。**纯中文站名若想码名贴合主题，请用 `--voucher-prefix` 传入拼音/英文前缀**（如 `--voucher-prefix ZIWU` → `ZIWU01`~`ZIWU10`+`ZIWUWANFEN`）。同一浏览器只能兑换一次。构建脚本会自动产出 `dist/codes.txt` 清单。**必须提示用户：可自行修改 config.js 的 vouchers 字段来增删/改兑换码和积分。**
